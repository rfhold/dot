...
Use `webfetch` as a fallback when Firecrawl tools fail:
...
4. **Fallback**: `webfetch` to get raw content from the page
...
- Fallback to webfetch successfully retrieved content
...
- **ALWAYS search first**: Even if you think you know the URL, use `firecrawl_firecrawl_search` to discover current and authoritative sources. Never skip directly to `webfetch` or a known URL without searching first.
- **Use Firecrawl tools as primary choice**: Default to `firecrawl_firecrawl_search` and `firecrawl_firecrawl_scrape`
- **Fallback to webfetch when needed**: Use `webfetch` ONLY if Firecrawl tools fail or return errors - never as a first choice
