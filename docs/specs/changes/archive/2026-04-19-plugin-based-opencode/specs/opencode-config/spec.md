# opencode-config Delta Spec

## Change Overview

### Why

OpenCode capabilities — skills, agent prompts, MCP server wiring, and custom commands — are currently delivered through three overlapping mechanisms in `~/dot`:

1. Skills cloned into `~/.agents/skills/` (global) or a filtered `.agents/skills/` tree generated per org by `configure.py` from a rfhold/cfaintl monorepo clone.
2. MCP servers declared as inline `mcp` stanzas inside `opencode.jsonc` (global and org-scoped).
3. Plugin files dropped directly into `~/.config/opencode/plugins/` (e.g. `cuthulu.js`, `shell-env.js`, `anthropic-oauth.ts`), and agent prompts stored as loose `agent/*.md` files in the config dir.

This model has drifted: skills describing a specific repository (`cuthulu`, `gitops-query`, `homelab`, `walter`, etc.) live in a separate monorepo far from the code they describe; a query-server's MCP wiring and the skill that teaches the model to use that MCP live in different repos; broken symlinks (`oc-agent`, `oc-skill`, `atlassian-query`) accumulate in `~/.agents/skills/`; bundled plugin files like `cuthulu.js` must be manually copied into dot to be installed.

OpenCode already supports richer plugin semantics — plugins may register skills (via `config.skills.paths`), MCP servers (via `config.mcp`), agents (via `config.agent`), and commands (via `config.command`). The `superspec` plugin (`rfhold/superspec`) demonstrates this shape in production. Consolidating on plugin delivery makes each source repository the single owner of everything OpenCode needs to consume its capability, collapses three delivery mechanisms into one, and lets `opencode.jsonc` become a short declaration of which plugins are active at each layer.

### Impact

- **Breaking changes**:
  - Skill folder relocations: skills that describe a specific source repo move out of `rfhold/skills` and `~/dot/home/.agents/skills/` into the repo they describe. Consumers that hard-code paths under `~/repos/rfhold/.agents/skills/<name>/` or `~/.agents/skills/<name>/` will see those paths disappear (plugin-loaded skills are resolved inside plugin package directories, not via symlinks under `.agents/skills`).
  - Inline `mcp` stanzas in `opencode.jsonc` are dropped for plugin-owned MCP servers; operators who previously edited those stanzas directly must now edit the owning plugin.
  - `~/.config/opencode/plugins/cuthulu.js` is removed; the cuthulu plugin installs via `plugin` array instead.
- **Migration**:
  - Each query-server repo adds a `.opencode/plugins/<name>.js` plus a `skills/` tree (see requirements below).
  - `rfhold/skills` gains its own plugin and sheds relocated skills.
  - Global and rfhold-org `opencode.jsonc` are rewritten to declare the new plugin list.
  - Broken symlinks under `~/.agents/skills/` (`oc-agent`, `oc-skill`, orphan `atlassian-query/`) are removed.
  - The loose cuthulu plugin file is deleted from dot.
- **Cross-change dependencies**: none. This change is self-contained in `~/dot` plus the owning plugin repos.

### Non-goals

- Changing `cfaintl` org configuration. The cfaintl flow (npm registry, branch, skills_whitelist) remains as-is.
- Publishing plugins to any npm registry. All plugin installs use `git+ssh` specifiers.
- Replacing the `anthropic-oauth`, `shell-env`, `md-table-formatter`, or `dcp` plugins. They continue to install via their existing mechanisms.
- Migrating `~/.config/opencode/command/*.md`, `~/.config/opencode/agent/research.md` (except the research agent, which moves with its MCP), and the custom tools under `~/.config/opencode/tool/`. A later change may plugin-ize these; this change does not.
- Defining how plugins are *built* or versioned. Each owning repo decides its own commit discipline; `opencode.jsonc` tracks `main` via `git+ssh` URLs (same as the existing `superspec` entry).
- Adding a production MCP endpoint set. Preview URLs remain in use.
- Introducing tag-pinning, release workflows, or lockfile management for plugins.

### Rollback

Each stage is reversible:
1. Revert the relevant plugin repo commits (plugin files + skills subdirectories).
2. Revert `opencode.jsonc` in `~/dot/.config/opencode/` and `~/repos/rfhold/.agents/` to reintroduce inline `mcp` stanzas and drop plugin entries.
3. Re-run `configure.py` with the previous `ORG_SKILLS` shape to regenerate the filtered `.agents/skills/` tree.
4. Restore `~/.config/opencode/plugins/cuthulu.js` from git history.

No runtime data is written or migrated; rollback is purely file restoration.
...
Given `rfhold/cuthulu` ships its OpenCode plugin at `.opencode/plugins/cuthulu.js`
...
The system MUST install every plugin by listing it in an `opencode.jsonc` `plugin` array using a `git+ssh` specifier pointing at the plugin's source repository.
...
Then the entry MUST be of the form `<name>@git+ssh://git@git.holdenitdown.net/rfhold/<repo>.git`
