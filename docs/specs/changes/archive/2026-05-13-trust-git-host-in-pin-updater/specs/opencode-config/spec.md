...
- **Why**: The OpenCode plugin pin updater now has SSH tooling, but `git+ssh` tag lookups still fail in CI when SSH cannot verify `git.holdenitdown.net` as a trusted host.
- **Impact**: The updater workflow will establish SSH host trust for the Forgejo host before resolving rfhold plugin tags.
...
- **Rollback**: Remove the updater workflow host trust setup and return to the previous SSH environment behavior.
